...
def make_subplot_for_all_probs(
    init_domains,
    ...
    fig = subplot_layout(fig, init_domains, 1, 1, "Initial Layout", label_shapes=True,  xrange=xrange, yrange=yrange)

    for ix, (row, col) in enumerate(indices[1:]):
        try:
            res = results[ix]
            label = f"{ix}. {res.display_message()}"
            fig = subplot_layout(
                fig, res.layout.domains, row, col, label, xrange, yrange, label_shapes=True
            )
        except IndexError:
            # out of solutions
            break
    ...
    if save_location:
        fig.write_image(save_location / "fixer_process.png", scale=5, width=1200, height=900,)
    return fig
