...
from helpers.directions import Direction, get_axis, get_opposite_axis
...
def isolate_node_and_drn(name: str, drns: list[str]) -> list[NodeEdgeDrn]: # TODO become aware of the type.. 
...
def group_nodes_on_edge(G: nx.Graph) -> list[list[NodeEdgeDrn]]:
    ...
    edge_nodes_and_drn = chain_flatten(isolate_node_and_drn(*i) for i in res if i.empty_drns)
...
def sort_nodes_on_egde(G: nx.Graph, domains:Dict[str, Domain]) ->  Dict[str, list[Domain]]:
