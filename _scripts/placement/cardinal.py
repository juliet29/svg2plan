...
def handle_missing_nodes(G: nx.DiGraph, domains: DomainsDict, ax: AxisNames):
    ...
    def create_x_edges(node):
        return [(Direction.WEST.name, node), (node, Direction.EAST.name)]

    def create_y_edges(node):
        return [(Direction.SOUTH.name, node), (node, Direction.NORTH.name)]
...
def create_dag_with_cardinal_directions(
    G: nx.DiGraph, domains: DomainsDict, ax: AxisNames
):
    drn1, drn2 = (
