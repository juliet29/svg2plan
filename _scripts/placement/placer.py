...
class Placer:
    ...
    def place_next_west_node(self, east_node):
        new_y_top = 0
        new_x_left = self.lo.domains[east_node].new_corners.x_right 
        self.create_new_corners(self.lo.curr_node, new_x_left, new_y_top)


    def place_next_south_node(self, north_node, east_node):
        new_y_top = self.lo.domains[north_node].new_corners.y_bottom
        if east_node:
            if self.lo.domains[east_node].new_corners == Corners(0,0,0,0):
                print(f"{east_node}, the east node of {self.lo.curr_node} has not yet been placed.")
